...
# Diccionario de mínimos críticos solicitados
MINIMOS_DEFAULT = {
...
# --- 4. INTERFAZ PRINCIPAL (AUTENTICADO) ---
...
st.sidebar.divider()
# Lectura de stock para alertas
df_alertas = pd.read_csv(STOCK_FILE)
alertas = df_alertas[df_alertas['Cantidad'] <= df_alertas['Minimo']]
if not alertas.empty:
    st.sidebar.error("⚠️ STOCK BAJO MÍNIMOS")
    for _, fila in alertas.iterrows():
        st.sidebar.warning(f"{fila['Vacuna']}: {int(fila['Cantidad'])} (Mín: {fila['Minimo']})")
...
tab_reg, tab_hist, tab_graf, tab_conf = st.tabs(["📝 Registro", "📋 Historial", "📊 Estadísticas", "⚙️ Gestión de Stock"])

# TAB 1: REGISTRO Y DESCUENTO
with tab_reg:
    ...
    with col_c:
        ...
        if st.session_state.cesta:
            ...
            if st.button("✅ GUARDAR Y DESCONTAR STOCK"):
                ...
                st.rerun()
        else: st.info("Cesta vacía")

# TAB 2: HISTORIAL Y DEVOLUCIÓN DE STOCK
with tab_hist:
    st.subheader("📋 Registro de Actividad")
    df_ver = pd.read_csv(DB_FILE)
    if not df_ver.empty:
        df_display = df_ver.iloc[::-1].copy()
        id_eliminar = st.selectbox("Seleccionar registro para eliminar (devuelve stock):", 
        ...
        if st.button("🗑️ Eliminar y Devolver Dosis"):
            ...
            if vacuna_retorno in df_stock_ret['Vacuna'].values:
                df_stock_ret.loc[df_stock_ret['Vacuna'] == vacuna_retorno, 'Cantidad'] += 1
...
with tab_conf:
    ...
    with col_st1:
        st.write("### Ajuste Manual de Stock")
        v_a = st.selectbox("Vacuna a modificar:", df_st['Vacuna'])
        n_a = st.number_input("Cantidad a sumar (+5) o restar (-5):", step=1)
        if st.button("Actualizar Unidades"):
            df_st.loc[df_st['Vacuna'] == v_a, 'Cantidad'] += n_a
    ...
    v_borrar = st.selectbox("Vacuna a eliminar:", list(st.session_state.lista_vacunas.keys()))
